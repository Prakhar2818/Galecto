        ...
        <div className="col-span-8 space-y-12">
          ...
          <section>
            <h3 className="text-xl font-black font-sora mb-6 flex items-center gap-3">
              <lucide_react_1.Zap className="text-emerald-500"/> Quick Start
            </h3>
            <div className="bg-slate-900 rounded-[2.5rem] p-10 text-white relative overflow-hidden">
               ...
               <div className="relative z-10">
                  <div className="flex justify-between items-center mb-6">
                     ...
                     <button onClick={() => copyCode('node', nodeCode)} className="text-xs font-bold text-slate-400 hover:text-white flex items-center gap-2 transition-colors">
                        {copied === 'node' ? <lucide_react_1.Check size={14} className="text-emerald-500"/> : <lucide_react_1.Copy size={14}/>}
          ...
          <section>
            <h3 className="text-xl font-black font-sora mb-6 flex items-center gap-3">
              <lucide_react_1.Terminal className="text-blue-500"/> HTTP API Reference
            </h3>
            <div className="space-y-6">
               <ApiEndpoint method="POST" path="/api/v1/ingest" desc="Ingest raw telemetry data (logs, traces, errors)." code={curlCode} copied={copied === 'curl'} onCopy={() => copyCode('curl', curlCode)}/>
               <ApiEndpoint method="GET" path="/api/v1/traces/:id" desc="Retrieve the full causality tree for a specific trace."/>
            </div>
          </section>
        </div>
        ...
        <div className="col-span-4 space-y-8">
           <div className="bg-white p-8 rounded-[2.5rem] border border-slate-100 shadow-sm">
              ...
              <div className="space-y-4">
                 <SdkItem icon={<lucide_react_1.Code2 className="text-blue-500"/>} name="Node.js SDK" version="v1.2.0"/>
                 <SdkItem icon={<lucide_react_1.Server className="text-orange-500"/>} name="Go Package" version="Coming Soon" disabled/>
                 <SdkItem icon={<lucide_react_1.Box className="text-blue-400"/>} name="Python Wrapper" version="Coming Soon" disabled/>
              </div>
           </div>

           <div className="bg-emerald-500 p-8 rounded-[2.5rem] text-white">
              <lucide_react_1.ShieldCheck size={32} className="mb-4 opacity-50"/>
...
function SdkItem({ icon, name, version, disabled }) {
    return (<div className={`flex items-center justify-between p-4 rounded-2xl border border-slate-50 ${disabled ? 'opacity-40 grayscale' : 'hover:bg-slate-50 transition-colors cursor-pointer'}`}>
       ...
       {!disabled && <lucide_react_1.Book size={16} className="text-slate-300"/>}
    </div>);
}
function ApiEndpoint({ method, path, desc, code, copied, onCopy }) {
    return (<div className="bg-white rounded-[2rem] border border-slate-100 overflow-hidden shadow-sm">
       ...
       <div className="p-6">
          ...
          {code && (<div className="bg-slate-50 p-4 rounded-xl relative group">
               <button onClick={onCopy} className="absolute top-4 right-4 text-slate-300 hover:text-slate-900 opacity-0 group-hover:opacity-100 transition-opacity">
                  {copied ? <lucide_react_1.Check size={14} className="text-emerald-500"/> : <lucide_react_1.Copy size={14}/>}
               </button>
