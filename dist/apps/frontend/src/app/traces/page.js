...
function TracesPage() {
    ...
    const [traceTree, setTraceTree] = (0, react_1.useState)(null);
    const [loadingTrace, setLoadingTrace] = (0, react_1.useState)(false);
    const fetchData = async () => {
        setLoading(true);
        try {
            const data = await (0, apiClient_1.queryFetch)('/api/v1/traces');
            if (data.success)
                setTraces(data.data);
        }
        catch (err) {
    ...
    const handleViewTrace = async (traceId) => {
        ...
        try {
            const data = await (0, apiClient_1.queryFetch)(`/api/v1/traces/${traceId}`);
            if (data.success)
    ...
    return (<DashboardLayout_1.default>
      <div className="flex justify-between items-end mb-10">
        ...
        <div className="flex gap-3">
          <button onClick={fetchData} className="btn-secondary !py-2.5 !px-5 !rounded-xl text-sm flex items-center gap-2">
            Refresh Stream <lucide_react_1.RefreshCw className={`w-4 h-4 ${loading ? 'animate-spin' : ''}`}/>
          </button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-12 gap-8">
          ...
          <div className="bg-white rounded-[2.5rem] shadow-sm border border-slate-100 overflow-hidden">
            <div className="p-8 border-b border-slate-50 flex justify-between items-center bg-slate-50/30">
              <div className="flex items-center gap-3">
                <lucide_react_1.Search size={16} className="text-slate-400"/>
                <span className="text-xs font-bold uppercase tracking-widest text-slate-400">Live Ingestion</span>
              </div>
              <lucide_react_1.Filter size={16} className="text-slate-400 cursor-pointer hover:text-emerald-500 transition-colors"/>
            </div>
            <div className="overflow-y-auto max-h-[700px]">
              {loading && traces.length === 0 ? (<div className="p-20 flex flex-col items-center justify-center gap-4">
                ...
                </div>) : traces.map((t) => (<div key={t.trace_id} onClick={() => handleViewTrace(t.trace_id)} className={`p-8 border-b border-slate-50 cursor-pointer transition-all hover:bg-slate-50 group relative ${selectedTrace === t.trace_id ? 'bg-emerald-50/50 border-l-4 border-l-emerald-500' : ''}`}>
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex gap-2">
                      {t.services.map((s) => (<span key={s} className="px-2 py-0.5 bg-white border border-slate-100 text-[8px] font-black uppercase rounded-md text-slate-500">{s}</span>))}
                    </div>
                    <div className="text-xs font-bold text-slate-400">{t.event_count} events</div>
                  </div>
                </div>))}
              {!loading && traces.length === 0 && (<div className="p-20 text-center text-slate-400 font-bold text-sm uppercase tracking-widest">
        ...
        {selectedTrace && (<div className="col-span-8 animate-in slide-in-from-right-10 duration-700">
             <div className="bg-white p-8 rounded-[2.5rem] shadow-2xl border border-emerald-100 h-[800px] flex flex-col relative overflow-hidden">
                ...
                <div className="flex-grow bg-slate-50/50 rounded-[2rem] border border-slate-100 relative overflow-hidden">
                   {loadingTrace ? (<div className="absolute inset-0 flex items-center justify-center bg-white/40 backdrop-blur-sm z-20">
                         <div className="flex flex-col items-center gap-4">
                            <lucide_react_1.RefreshCw className="w-8 h-8 text-emerald-500 animate-spin"/>
                            <span className="text-xs font-bold text-emerald-600 uppercase tracking-widest">Stitching spans...</span>
                         </div>
                      </div>) : <TraceGraph_1.default tree={traceTree || []}/>}
                </div>
                ...
                <div className="mt-8 flex gap-4">
                   <div className="flex-grow p-6 bg-emerald-500 rounded-3xl text-white flex items-center justify-between shadow-lg shadow-emerald-500/20">
                      ...
                      <lucide_react_1.Activity className="w-8 h-8 opacity-40"/>
                   </div>
