...
function SettingsPage() {
    ...
    const [retentionDays, setRetentionDays] = (0, react_1.useState)(30);
    const [retentionLoading, setRetentionLoading] = (0, react_1.useState)(false);
    const fetchProjects = async () => {
        setLoading(true);
        try {
            const data = await (0, apiClient_1.apiFetch)('/projects');
            if (data.success)
    ...
    const fetchRetentionSettings = async () => {
        try {
            const data = await (0, apiClient_1.apiFetch)('/organization/settings');
    ...
    const handleCreateProject = async (e) => {
        ...
        if (!newProjectName)
            return;
        try {
            await (0, apiClient_1.apiFetch)('/projects', {
                method: 'POST',
    ...
    const handleRetentionUpdate = async () => {
        ...
        try {
            await (0, apiClient_1.apiFetch)('/organization/settings', {
                method: 'PUT',
    ...
    return (<DashboardLayout_1.default>
      ...
      <div className="grid grid-cols-12 gap-12">
        ...
        <div className="col-span-3 space-y-2">
           <SettingsNavItem icon={<lucide_react_1.User size={18}/>} label="General Account" active/>
           <SettingsNavItem icon={<lucide_react_1.FolderOpen size={18}/>} label="Projects"/>
           <SettingsNavItem icon={<lucide_react_1.Key size={18}/>} label="API Integration"/>
           <SettingsNavItem icon={<lucide_react_1.Shield size={18}/>} label="Security & Access"/>
           <SettingsNavItem icon={<lucide_react_1.Database size={18}/>} label="Data Retention"/>
        </div>
           ...
           <div className="bg-white p-10 rounded-[2.5rem] shadow-sm border border-slate-100">
              <div className="flex justify-between items-center mb-8">
                 ...
                 <form onSubmit={handleCreateProject} className="flex gap-2">
                    <input type="text" placeholder="Project Name..." className="px-4 py-2 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:outline-none focus:border-emerald-500" value={newProjectName} onChange={(e) => setNewProjectName(e.target.value)}/>
              ...
              <div className="space-y-6">
                 {loading ? (<div className="py-10 flex flex-col items-center gap-3">
                   ...
                   </div>) : projects.length === 0 ? (<div className="py-10 text-center text-slate-400 font-medium italic">No projects found. Create your first one above!</div>) : projects.map((project) => (<div key={project.id} className="p-8 bg-slate-50 rounded-[2rem] border border-slate-100 group">
                      <div className="flex justify-between items-center mb-6">
                         <div className="flex items-center gap-3">
                            <div className="w-10 h-10 bg-white rounded-xl flex items-center justify-center shadow-sm">
                               <lucide_react_1.FolderOpen className="w-5 h-5 text-emerald-500"/>
                            </div>
                            <div>
                      ...
                      <div className="space-y-3">
                         {project.apiKeys?.map((apiKey) => (<div key={apiKey.id} className="flex items-center gap-4 bg-white p-4 rounded-2xl border border-slate-100 group/key">
                              ...
                              <button onClick={() => copyToClipboard(apiKey.key)} className="p-2 hover:bg-slate-50 rounded-lg transition-colors text-slate-400 hover:text-emerald-500">
                                 {copiedKey === apiKey.key ? <lucide_react_1.Check size={16}/> : <lucide_react_1.Copy size={16}/>}
                              </button>
                           </div>))}
                      </div>
                   </div>))}
    ...
    {/* Section 3: Data Retention */}
            <div className="bg-white p-10 rounded-[2.5rem] shadow-sm border border-slate-100">
               <div className="flex items-center gap-4 mb-2">
                  <lucide_react_1.Database className="text-orange-500" size={24}/>
               ...
               <div className="flex items-center gap-8 p-8 bg-slate-50 rounded-3xl border border-slate-100">
                  ...
                  <div className="flex items-center gap-4">
                     <select className="px-6 py-3 bg-white border border-slate-200 rounded-2xl font-bold text-sm focus:outline-none focus:border-emerald-500" value={retentionDays} onChange={(e) => setRetentionDays(Number(e.target.value))}>
                        <option value="7">7 Days</option>
                        <option value="14">14 Days</option>
                        <option value="30">30 Days</option>
                        <option value="90">90 Days</option>
                     </select>
                     <button onClick={handleRetentionUpdate} disabled={retentionLoading} className="btn-primary !py-3 !px-8">
...
function SettingsNavItem({ icon, label, active }) {
    return (<div className={`flex items-center gap-4 px-6 py-4 rounded-2xl cursor-pointer transition-all ${active ? 'bg-white text-emerald-600 font-bold shadow-sm border border-slate-100' : 'text-slate-400 hover:bg-white hover:text-slate-900 border border-transparent hover:border-slate-100'}`}>
      <div className={active ? 'text-emerald-500' : ''}>{icon}</div>
      <span className="text-sm">{label}</span>
      {active && <lucide_react_1.ChevronRight size={16} className="ml-auto opacity-50"/>}
    </div>);
}
function UserRow({ name, role, email }) {
    return (<div className="flex items-center justify-between p-6 bg-slate-50 rounded-3xl hover:bg-slate-100 transition-colors cursor-pointer group">
       <div className="flex items-center gap-4">
