    ...
    <DashboardLayout>
      ...
      <div className="grid grid-cols-4 gap-6 mb-10">
        ...
        <StatCard
          ...
          value={metrics.length > 0 ? metrics.length.toString() : '0'}
          trend="Operational"
          color="bg-white"
                    ...
                    traces.slice(0, 5).map((t) => (
                      <tr key={t.trace_id} className="hover:bg-slate-50 transition-colors">
                        <td className="px-8 py-6">
                          ...
                          <div className="text-[10px] text-slate-400 font-bold">{new Date(t.start_time).toLocaleString()}</div>
                        </td>
                        <td className="px-8 py-6">
                        ...
                        <td className="px-8 py-6">
                          <div className="flex gap-1.5 flex-wrap">
                            {(t.services || []).slice(0, 2).map((s: string) => (
                              <span key={s} className="px-2 py-0.5 bg-slate-100 text-slate-500 text-[8px] font-black uppercase rounded-md">
                                {s}
                              </span>
                            ))}
                          </div>
